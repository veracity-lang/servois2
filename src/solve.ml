...
let define_fun (name : string) (args : ty bindlist) (r_ty : ty) (def : exp) : string =
    "(define-fun " ^
    name ^ "\n" ^
    "  ( " ^ String.concat "\n    " (List.map (fun (Var s, ty) -> s ^ " " ^ string_of_ty ty) args) ^ " )\n" ^
    "  " ^ string_of_ty r_ty ^ "\n" ^
...
let smt_of_spec spec = (* TODO: Preamble? *)
    ...
    String.concat "" (List.map (fun (m : method_spec) ->
        ...
        define_fun (m.name ^ "_post_condition") (s @ m.args @ s_new) TBool m.post ^ "\n"
    ) spec.methods) ^
...
let generate_bowtie spec m1 m2 = 
    ...
    let post_args_list old_postfix new_postfix argslist = String.concat " "
        (("result" ^ new_postfix) ::
         ...
         argslist @
         List.map (fun a -> a ^ new_postfix) datanames) in
    ...
    let oper_xy x y mname args = "  (" ^ mname ^ "_pre_condition " ^ pre_args_list x args ^ ")\n" ^
        "  (" ^ mname ^ "_pre_condition " ^ post_args_list x y args ^ ")\n" in
    ...
    List.fold_left (fun acc k -> acc ^ "(= result_" ^ string_of_int k ^ "_1 result_" ^ string_of_int k ^ "21)\n   ") "" (flip List.init succ @@ List.length m1.ret) ^
    List.fold_left (fun acc k -> acc ^ "(= result_" ^ string_of_int k ^ "_2 result_" ^ string_of_int k ^ "12)\n   ") "" (flip List.init succ @@ List.length m2.ret) ^
    "   (states_equal " ^
