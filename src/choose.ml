...
type choose_env =
  ...
  ; commute_fn : spec -> conjunction -> exp
  ; non_commute_fn : spec -> conjunction -> exp
  ; spec : spec
  ; m_spec: method_spec
  ; n_spec: method_spec
  ; h : conjunction
...
let poke env : predP =
    ...
    let weight_fn p =
        ...
        let weight_fn_inner h_com h_ncom = match env.solver smt_diff_preds @@ env.commute_fn env.spec h_com with
            | Unsat -> 0
            | Unknown -> List.length smt_diff_preds
            | Sat vs -> begin let com_cex = pred_data_of_values vs in
                match env.solver smt_diff_preds @@ env.non_commute_fn env.spec h_ncom with
                | Unsat -> 0
                | Unknown -> List.length smt_diff_preds
...
let poke2_weight_fn env diff_preds =
  let smt_diff_preds = List.map exp_of_predP diff_preds in fun p cov ->
        ...
        match env.solver smt_diff_preds @@ env.commute_fn env.spec h' with
        | Unsat -> begin match env.solver smt_diff_preds @@ env.non_commute_fn env.spec h'' with
            | Unsat -> -1
            | Unknown -> List.length smt_diff_preds
            | Sat s -> 0
            end
        ...
        | Sat vs -> begin let com_cex = pred_data_of_values vs in
            match env.solver smt_diff_preds @@ env.non_commute_fn env.spec h'' with
            | Unsat -> 0
            | Unknown -> List.length smt_diff_preds
            | Sat vs -> begin let non_com_cex = pred_data_of_values vs in
                differentiating_predicates_sym diff_preds com_cex non_com_cex |> List.length
