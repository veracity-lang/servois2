...
class TokenSubmap(log: Record.Log, msg : Message, initialTokens: Int) extends Contract(log) {
  ...
  val allowed: Mapping[Address, Map[Address, Int]] = new Mapping(this, "allowed", TrieMap())
      ...
      balances.get(msg, from) match {
        case Some(from_balance: Int) =>
          ...
          allowed.get(from) match {
            case Some(from_allowed: Map[Address, Int]) =>
              from_allowed.get(to) match {
                case Some(allowed_by_from: Int) =>
                  ...
                  from_allowed.put(from, allowed_by_from - value)
    ...
    addToLog(Begin(msg, name, "approve(" + msg + ", " + spender + ", " + value + ")"))
      ...
      val from_allowed = allowed.get(msg.getSender()) match {
        case Some(from_allowed: Map[Address, Int]) =>
          from_allowed
        case None =>
          val mapping = Map[Address, Int]()
          allowed.put(msg, msg.getSender(), mapping)
          mapping
      }

      // allowed[msg.sender][_spender] = _value;
      from_allowed.put(spender, value)
  ...
  def allowance(msg: Message, owner: Address, spender: Address) : Int = {
      ...
      allowed.get(owner) match {
        case Some(from_allowed: Map[Address, Int]) =>
          from_allowed.get(spender) match {
  ...
  def increaseApproval(msg: Message, spender: Address, addedValue: Int) : Boolean = {
    addToLog(Begin(msg, name, "increaseApproval(" + msg + ", " + spender + ", " + addedValue + ")"))
      ...
      val previousAmount = from_allowed.get(spender) match {
        case Some(amount: Int) =>
          amount
        case None =>
          0
      ...
      from_allowed.put(spender, previousAmount + addedValue)
